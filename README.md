...
```bash
git clone https://github.com/you/remote-vibes.git
cd remote-vibes
...
This starts:
- `rv_postgres` — PostgreSQL 16
- `rv_redis` — Redis 7
- `rv_main` — FastAPI app on port **8000**
...
| `AGENT_IMAGE` | No | Docker image for agent containers (default: `rv_agent:latest`) |
...
FastAPI calls DockerManager.start_agent_container()
         ↓
New Docker container: rv_agent:latest
